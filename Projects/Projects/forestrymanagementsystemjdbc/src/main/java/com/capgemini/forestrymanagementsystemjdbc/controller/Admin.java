...
import com.capgemini.forestrymanagementsystemjdbc.exception.ProductException;
...
import com.capgemini.forestrymanagementsystemjdbc.validation.CodeValidation;
...
public class Admin {
	...
	public static void addCustomer() {
		...
		do {
			...
			if (CodeValidation.mobile(mob)) {
				count++;
				bean.setTelno(mob);
			} else {
				System.err.println("Enter 10 digit mobile number/try number format");
			}
		} while (count == 0);
		...
		do {
			...
			if (CodeValidation.email(email)) {
				...
				System.err.println("Enter email in correct format");
			}
		} while (count == 0);
	...
	public static void searchHaulier() {
		...
		if (haulierService.searchHaulier(haulierid)) {
			...
			System.err.println("No no haulier found");
		}
	}
