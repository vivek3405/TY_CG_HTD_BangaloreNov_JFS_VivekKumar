							...
							while (count1 == 0) {
								...
								case 1:
									...
									while (temp == 0) {
										...
										name = sc.next();
										if (CodeValidation.name(name)) {
											temp++;
										} else {
											System.out.println(
													"You can not use any special character or number in the name");
										}
									}
				...
				case 1:
					...
					while (temp == 0) {
						...
						if (CodeValidation.name(name)) {
							...
							System.out.println("You can not use any special character or number in the name");
						}
					}
					bean.setCustomerName(name);
					...
					while (temp == 0) {
						try {
							...
							if (CodeValidation.mobile(number)) {
								temp++;
							} else {
								System.out.println(
										"Mobile number should be of 10 digit\nMobile number Should Start from 6,7,8 and 9 only");
							}
						} catch (Exception e) {
							System.err.println("You can enter only numeric values");
						}
					}
					bean.setMobileNo(number);
							...
							case 1:
								if (!productMap.isEmpty()) {
									...
									for (Entry<Integer, List<ProductBean>> entry : entries) {
										...
										System.out.println(entry.getValue());
										System.out.println("-------------------");
										System.out.println("Enter product ID to add the product to Your 'Wishlist'");
										int prodid = sc.nextInt();
